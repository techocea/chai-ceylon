...
interface PackageModalProps {
  ...
  onSubmit: (formData: PackageData) => void;
}
...
const PackageModal = ({
  ...
  const [formData, setFormData] = useState<
    Omit<PackageData, "imageUrl" | "packageTypeId">
  >({
    name: "",
    price: 0,
    description: "",
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setPackageTypeId(initialData.packageTypeId);
    } else {
      setFormData({
        name: "",
        price: 0,
  ...
  return (
    <div
      className={`fixed ${
        isUpdateMode ? "pt-48" : "pt-0"
      } inset-0 bg-black/50 backdrop-opacity-50 overflow-y-auto w-full h-full z-20 flex justify-center items-center`}
    >
      <div className="relative left-1/6 -translate-x-1/6 p-8 border w-full max-w-lg mx-4 shadow-lg rounded-md bg-white my-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* CATEGORY DROPDOWN */}
          <div>
            <Label htmlFor="packageTypeId">Select Package Type</Label>
            <select
              name="packageTypeId"
              value={packageTypeId}
          ...
          {/* PACKAGE PRICE & isAvailable CHECKBOX */}
